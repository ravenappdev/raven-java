...
@JsonDeserialize(
    using = GetDeviceException.Deserializer.class
)
public final class GetDeviceException extends Exception {
  private final Value value;

  private int statusCode;

  private GetDeviceException(Value value, int statusCode) {
    this.value = value;
  ...
  public static GetDeviceException other(Object unknownValue, int statusCode) {
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "GetDeviceException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<GetDeviceException> {
    @Override
    public GetDeviceException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new GetDeviceException(value, statusCode);
    }
  }
}
