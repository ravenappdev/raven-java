package com.raven.api.client.event.exceptions;
...
@JsonDeserialize(
    using = SendBulkException.Deserializer.class
)
public final class SendBulkException extends Exception {
  private final Value value;

  private int statusCode;

  private SendBulkException(Value value, int statusCode) {
    this.value = value;
  ...
  public static SendBulkException other(Object unknownValue, int statusCode) {
    return new SendBulkException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "SendBulkException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<SendBulkException> {
    @Override
    public SendBulkException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new SendBulkException(value, statusCode);
    }
  }
}
