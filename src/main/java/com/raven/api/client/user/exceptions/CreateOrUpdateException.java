package com.raven.api.client.user.exceptions;
...
@JsonDeserialize(
    using = CreateOrUpdateException.Deserializer.class
)
public final class CreateOrUpdateException extends Exception {
  private final Value value;

  private int statusCode;

  private CreateOrUpdateException(Value value, int statusCode) {
    this.value = value;
  ...
  public static CreateOrUpdateException other(Object unknownValue, int statusCode) {
    return new CreateOrUpdateException(new UnknownErrorValue(unknownValue), statusCode);
  }
  ...
  private static final class UnknownErrorValue implements Value {
    ...
    public String toString() {
      return "CreateOrUpdateException{" + "unknownValue: " + unknownValue + "}";
    }
  }

  static final class Deserializer extends JsonDeserializer<CreateOrUpdateException> {
    @Override
    public CreateOrUpdateException deserialize(JsonParser p, DeserializationContext ctx) throws
        IOException {
      Value value = ctx.readValue(p, Value.class);
      int statusCode = (int) ctx.getAttribute("statusCode");
      return new CreateOrUpdateException(value, statusCode);
    }
  }
}
